...
from ..firewall_os import remove_firewall_rule_os, add_firewall_rule_os

firewall_groups_router = APIRouter()  # prefix kaldırıldı
...
async def list_groups(admin=Depends(require_admin), db=Depends(get_database)):
...
async def create_group(data: FirewallGroupModel, admin=Depends(require_admin), db=Depends(get_database)):
    group_doc = {
...
async def delete_group(group_id: str, admin=Depends(require_admin), db=Depends(get_database)):
    try:
...
async def enable_disable_group(group_id: str, enable: bool, admin=Depends(require_admin), db=Depends(get_database)):
    try:
        obj_id = ObjectId(group_id)
    except:
        raise HTTPException(400, "Geçersiz group_id (ObjectId değil)")
...
async def list_group_rules(group_id: str, admin=Depends(require_admin), db=Depends(get_database)):
    try:
        obj_id = ObjectId(group_id)
    except:
