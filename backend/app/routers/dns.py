...
from ..dns_proxy_manager import dns_proxy_manager

dns_router = APIRouter()  # prefix kaldırıldı
...
async def list_blocked_domains(admin=Depends(require_admin), db=Depends(get_database)):
...
async def add_blocked_domain(domain_in: DomainBlock, admin=Depends(require_admin), db=Depends(get_database)):
...
async def remove_blocked_domain(domain: str, admin=Depends(require_admin), db=Depends(get_database)):
...
async def import_adblock_list(data: AdblockList, admin=Depends(require_admin), db=Depends(get_database)):
...
async def start_dns_proxy(config: DNSProxyConfig, admin=Depends(require_admin), db=Depends(get_database)):
    """DNS Proxy'yi başlat"""
    try:
        # Config'i kaydet
...
async def get_proxy_status(admin=Depends(require_admin), db=Depends(get_database)):
...
async def check_dns_proxy():
    ...
    try:
        ...
        print(f"Error checking DNS proxy on startup: {e}")